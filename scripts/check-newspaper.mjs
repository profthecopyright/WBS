...
assert.equal(hongbo.image, '/images/inside-gallery/hongbo-li-v4.png');
assert.ok(hongbo.paragraphs.slice(1).every((paragraph) => paragraph.includes('Meow') && !paragraph.includes('Hongbo')));
...
const currentPage = readFileSync('app/newspaper.tsx', 'utf8').replaceAll('&apos;', "'");
...
function checkFaqLinks(node) {
  if (ts.isJsxElement(node) && node.openingElement.tagName.getText() === 'Link') {
    ...
    const slug = href.text.replace(/^\/pros\//, '').replace(/\/$/, '');
...
for (const node of parse(original('app/page.tsx')).statements.filter(ts.isFunctionDeclaration)) {
  ...
  const expected = ['blogHref', 'blogPageHref'].includes(node.name?.text)
    ? node.getText().replaceAll('#blogs/${post.slug}', '/blogs/${post.slug}/').replaceAll('#blogs/page/${page}', '/blogs/page/${page}/').replaceAll('"#blogs"', '"/blogs/"')
    : node.getText();
  assert.equal(normalize(functions.get(node.name?.text)?.getText() ?? ''), normalize(expected), `Changed article helper: ${node.name?.text}`);
  preservedHelpers++;
}
...
const routes = readFileSync('app/site-routes.ts', 'utf8');
assert.ok(routes.includes('requestedHash === "lessons" ? "courses"'));
assert.ok(routes.includes('requestedHash === "inside"') && routes.includes('requestedHash === "front"'));
assert.ok(currentPage.includes('router.push'));
assert.ok(currentPage.includes('window.location.replace'));
